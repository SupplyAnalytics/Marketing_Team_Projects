...
def generate_pdf(df):
    ...
    for EmployeeId, group in df.groupby('EmployeeId'):
        ...
        default_width = 40  # Adjust the default width to allow more space for columns

        # Add a table header
        pdf.set_fill_color(200, 220, 255)
        for column, max_length in max_lengths.items():
            column_width = default_width  # Use default width for all columns
            pdf.cell(column_width, 10, column[:15], 1, 0, 'C', 1)  # Column headers (adjust width as needed)
        pdf.ln()
        ...
        for _, row in group.iterrows():
            for column in group.columns:
                ...
                pdf.multi_cell(column_width, 5, cell_text, border=1, align='L')  # Print cell value with text wrapping
